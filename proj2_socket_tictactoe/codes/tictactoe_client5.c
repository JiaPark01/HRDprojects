...
int main(int argc, const char* argv[])
{
    ...
    if (argc != 5) error_handling("./TICTACTOE_CLIENT5 serverIP 9999 nickname turn");    //* take turn as input to distinguish between 2 players (needs further dev in future)
    turn = atoi(argv[4]);
    if (turn != 1 && turn != 2) error_handling("turn must be 1 or 2");                  //* check for turn val for the ease of coding. getting num and using % may confuse players
    printf("Your turnID: %d\n\n", turn);
...
void* send_msg(void* args)
{
    ...
    while(true) {
        ...
        sprintf(name_msg, "%d %s\t: %s", turn, name, message);      //* even when 'q' is sent, the python client needs to know that the user has left, so send anyway
...
void* receive_msg(void* args)
{
    ...
    while (true) {
        ...
        char* raw_msg = strtok(name_msg, " ");                      //* separate message into turn, name, data
        printf("\nTurnID: %s\n", raw_msg);
