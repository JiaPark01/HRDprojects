...

{
    ...
    while (true) {
        ...
        if (detect_flag == true) {
            ...
            for (int i = 0; i < 2; ++i) {
                date_time.replace(date_time.find(":"), 1, "-");
            }

            std::vector<std::string> tokens = split(date_time, ' ');

            date_time = tokens[4];
            date_time.insert(date_time.size()-1, "_" + tokens[1]);
            date_time.insert(date_time.size()-1, "_" + tokens[2]);
            date_time.insert(date_time.size()-1, "_" + tokens[0]);
            date_time.insert(date_time.size()-1, "_" + tokens[3]);
            date_time.insert(date_time.size()-1, ".bmp");

            //date_time.erase(0, 1);
            //for (int i = 0; i < 4)
            
            std::cout << date_time << std::endl;
            cv::String _date_time = date_time;
            cv::imwrite(_date_time, frame);
